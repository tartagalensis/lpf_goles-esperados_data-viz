...
linea <- data.frame(x = c(3:9),y = c(3:9)) %>% 
  as_tibble() %>% 
  print()
...
p2 <- plot_xg %>% 
  ...
  theme_linedraw() +
  labs(title = "") + 
  geom_line(data = linea, aes(x = x, y = y), color = "blue") +
